...
function DeskCluster({ node }: { node: DeskNode }) {
  ...
  return (
    <group ref={groupRef} position={node.position}>
      ...
      <mesh position={[0, 0.748, 0.12]}>
        <planeGeometry args={[1.42, 0.12]} />
        <meshBasicMaterial color="#f7fafc" transparent opacity={0.16} />
      </mesh>
      <mesh position={[0, 0.746, -0.12]}>
        <planeGeometry args={[1.18, 0.04]} />
        <meshBasicMaterial color="#ffffff" transparent opacity={0.08} />
      </mesh>
      <mesh position={[0, 0.744, 0]}>
        <planeGeometry args={[0.42, 0.02]} />
        <meshBasicMaterial color="#dbeafe" transparent opacity={0.06} />
      </mesh>
      ...
      <mesh position={[0, 0.12, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <ringGeometry args={[0.88, 0.98, 36]} />
        <meshBasicMaterial color={[r, g, b]} transparent opacity={0.08} />
      </mesh>
      <mesh position={[0, 0.115, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <ringGeometry args={[1.04, 1.1, 36]} />
...
function DeskSeat({ position, active, accent }: { position: [number, number, number]; active: boolean; accent: THREE.Color | string }) {
  return (
  ...
  return (
    <group position={[0, 0, -4.64]}>
      ...
      <mesh position={[0, 0.78, 0.09]}>
        <planeGeometry args={[3.12, 0.005]} />
        <meshBasicMaterial color="#dbeafe" transparent opacity={0.035} />
      </mesh>
      <mesh position={[0, 0.64, 0.09]}>
        <planeGeometry args={[2.76, 0.004]} />
...
function AccentDisplay({ project, x }: { project: ProjectSummary; x: number }) {
  const { r, g, b } = hexToRgb(project.accent);
  ...
  return (
    <group position={[0, 1.72, 4.68]}>
      {projects.slice(0, 3).map((project, index) => {
        ...
        return <AccentDisplay key={project.id} project={project} x={x} />;
      })}
    </group>
  );
}
...
function WorldScene({ snapshot, mobileOptimized }: { snapshot: OfficeSnapshot; mobileOptimized?: boolean }) {
  ...
  return (
    <>
      ...
      <mesh position={[0, 0.56, 4.12]}>
        <planeGeometry args={[1.72, 0.03]} />
        <meshBasicMaterial color="#ffffff" transparent opacity={0.1} />
      </mesh>
      ...
      <mesh position={[-4.82, 0.54, -0.8]} rotation={[0, Math.PI / 2, 0]}>
        <planeGeometry args={[0.92, 0.02]} />
        <meshBasicMaterial color="#ffffff" transparent opacity={0.08} />
      </mesh>
      <mesh position={[4.86, 1.55, -0.8]} rotation={[0, -Math.PI / 2, 0]}>
